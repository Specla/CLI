...
class Methods {
  ...
  createController (action) {
    ...
    let controller = (req, res) => {
      try {
        let Controller = eval(`this.controllers.${controllerString}`) // eslint-disable-line
        let next = eval(`Controller.prototype.${action}.bind(Controller.prototype, req, res)`)  // eslint-disable-line
  ...
  validateMiddlewares (middlewares) {
    ...
    for (let middleware of middlewares) {
      ...
      correctedMiddlewares.push((req, res, next) => {
        try {
          eval(`this.middlewares.${middleware.replace(new RegExp('/', 'g'), '.')}(req, res, next)`) // eslint-disable-line
        } catch (err) {
